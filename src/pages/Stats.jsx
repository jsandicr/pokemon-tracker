...
import {
  Box, Typography, Card, CardContent, Grid, LinearProgress, Avatar,
  Button, Switch, FormControlLabel, useMediaQuery, useTheme
} from '@mui/material';
...
const DeckPerformanceRow = ({ deckName, winRate, totalMatches, pokemon1, pokemon2, trend, lastResults, isMobile }) => (
  <Box sx={{ mb: 3 }}>
    <Box sx={{ display: 'flex', mb: 1, justifyContent: 'space-between' }} flexDirection={isMobile ? "column" : "row"}>
      ...
      <Box gap={0.2} textAlign="right" display="flex" flexDirection="column" alignItems={isMobile ? "flex-start" : "flex-end"}>
              ...
              <Grid item xs={12} sm={4}>
                <Card sx={{ bgcolor: 'background.paper', borderRadius: 3, textAlign: 'center', p: 2 }}>
                  ...
                  <Typography variant="h6" fontWeight="bold" color={
                    stats.consistency?.status === 'Buen rendimiento' ? 'success.main' :
                    stats.consistency?.status === 'Necesita mejorar' ? 'error.main' :
                    'warning.main'
                  }>
